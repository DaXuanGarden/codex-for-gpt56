...
4. Explain that the repair creates a separate app and launcher. The default mode changes the active `$CODEX_HOME/config.toml` and can add default reasoning/service settings; when a trusted configuration manager owns that file, `--managed-config-repair` uses a generated isolated config and must not write the manager-owned global file.
...
- the state root, persistent model catalog, source global `config.toml`, and effective generated config path;
...
- when `--managed-updates` is requested, the managed-update plan/helper paths and the fact that future launcher starts can rebuild the copied app after the official source or copied bundle changes; if a trusted configuration manager such as CC Switch owns `config.toml`, separately disclose and request approval for `--managed-config-repair`, which refreshes an isolated config before launch without writing the global file.
...
- `modelCatalogPath` points to `$CODEX_HOME/model-catalogs/codex-for-gpt56/model-catalog.json`. In isolated mode, `sourceConfigPath` is the untouched manager-owned global file, `configPath` is under the state root, `launchCodexHome` is the generated home, and `managedConfigPolicy` is `isolated-catalog`.
...
When the user asks to remove the repair, inspect `managedConfigPolicy` first. For `isolated-catalog`, do not touch the global config because the repair never changed it. For other policies, use `configBackupPath` to restore only top-level values changed by this tool without overwriting unrelated later edits. Then remove the generated app, Desktop launcher, and state root. Remove the persistent model catalog only after no remaining config references it.
