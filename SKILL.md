...
- when `--managed-updates` is requested, the managed-update plan/helper paths and the fact that future launcher starts can rebuild the copied app after the official source or copied bundle changes; if a trusted configuration manager such as CC Switch rewrites `config.toml` at login, separately disclose and request approval for `--managed-config-repair`, which restores only `model_catalog_json` before launch.
...
- `configPath` points to `$CODEX_HOME/config.toml` and `modelCatalogPath` points to `$CODEX_HOME/model-catalogs/codex-for-gpt56/model-catalog.json`. On managed refresh, `configChanged` must be `false` unless the approved `repair-catalog` policy reconciled an externally changed `model_catalog_json`; in that case verify the backup and that unrelated config keys were preserved.
