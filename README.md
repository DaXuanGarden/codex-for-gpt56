...
- By default, backs up `$CODEX_HOME/config.toml`, sets its top-level `model_catalog_json`, and adds recommended reasoning/service defaults only when absent. When `--managed-config-repair` is selected for an external manager such as CC Switch, it does not write that global file: it creates a generated config under the state root from the manager's current config and overrides only `model_catalog_json` there.
...
| `--managed-config-repair` | Implies managed updates and isolates the copied app's config when CC Switch or another trusted manager owns the global file. Before launch, refreshes the generated config from the manager's current settings and overrides only the generated `model_catalog_json`; the global config is not written. |
...
- A persistent `modelCatalogPath`. In external-manager mode, `sourceConfigPath` points to the untouched global config, `configPath` and `launchCodexHome` point under the state root, and `managedConfigPolicy` is `isolated-catalog`.
...
Open the generated Desktop launcher or the root `.command`/`.cmd` launcher to use the isolated profile. Opening the copied app bundle directly bypasses both its isolated user-data and, in external-manager mode, its isolated `CODEX_HOME`. The copied app can require a separate sign-in.
...
2. If `managedConfigPolicy` is not `isolated-catalog`, read `configBackupPath` and restore only the top-level values changed by this tool. In isolated mode the global config was not changed, so no global-config restoration is needed.
...
Also remove the persistent catalog after confirming that neither the global nor generated config references it.
